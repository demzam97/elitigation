...
<table class="table table-bordered table-sm table-striped">
  ...
  <?php $i=1; foreach($usercases as $row):
    ...
    $appeal_reg = $this->public->get_appeal_registration_user($miscid);
    $disposalid = $this->public->get_disposal_id($miscid);
    ?>
   <tr>
      ...
      <td>
      <?php echo $this->public->get_disposal_name($disposalid);?>
      </td>
      <td>
      <?php 
       $i=1; 
      ...
      <td style="width: 5cm;"> 
      <?php 
      $appeal = $this->elat->get_appealcases_public($miscid);
      if(count($appeal) <= '0') { ?>
      <a  href="<?php echo site_url('elatappeal/appealcase_def/'.$miscid.''); ?>">Appeal Case</a>
      <?php } else  { ?>
       <font color = 'blue'>Appealed on:<br> <?php foreach($appeal as $ap) { $apid = $ap->id; echo $ap->created_on; }?><br>
       <a  href="<?php echo site_url('publicregistration/view_appealcase/'.$apid.''); ?>">View</a>
      <?php } ?>
      </td>
    </tr>
    <?php $i++; endforeach;?>
