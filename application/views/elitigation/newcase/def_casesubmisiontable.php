...
<!-- Tab Start -->
<div>
<h4>Judicial Form Submissions</h4>
<!-- Nav tabs -->
<ul class="nav nav-tabs" role="tablist">
  <li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Respondent / Defendant</a></li>
  <li role="presentation"><a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">Plaintiff / Petitioner / Applicant</a></li>
</ul>
...
<div class="tab-content">
  ...
  <?php $i=1; foreach($usercases as $row):?>
   <tr>
      ...
      $m=1; foreach($latigants as $lt):
            ...
            ?>
            <a href="<?php echo site_url('publicregistration/delete_case_files_def/'.$fld->id.'');?>" title="Delete" onclick="return confirm('Are you sure to Remove the File');">
      ...
      $m=1; foreach($latigants as $lt):
            ...
            if($fld->form_name_def == ''){
            ?>
       <a href="<?php echo site_url('publicregistration/upload_case_files_def/'.$fld->id.'/'.$lt->lit_id.'/'.$fld->judicial_process_id.'');?>"><i class="fa fa-upload" style="color:blue;"></i></a>
        <?php } echo "<br>"; }
           $j++; endforeach; } else { echo "No Judicial Process"; }
      endforeach;  
      ?>
      ...
      $m=1; foreach($latigants as $lt):
            ...
            if($jcf != '0'){
              ...
              if($fm->form_name_def == ''){ ?> <font color="red">Pending</font> <?php }
  ...
  <?php $i=1; foreach($usercases as $row):?>
   <tr>
      ...
      <?php 
      $miscid = $this->public->get_misc_case_id($row->id);
      $auid = $this->public->get_applicant_1_cid($miscid);
      $appcid = $this->public->get_applicant_cid($auid);
      $appuid = $this->public->get_defuid($appcid);
      $latigants = $this->public->get_litigantforms($miscid, $row->id);
      $m=1; foreach($latigants as $lt):
            $forms =$this->public->get_forms_cases($miscid);
            $jcf = count($forms);
            if($jcf != '0'){
            $j=1; foreach($forms as $fld):
              if($lt->lit_id == $appuid && $lt->judicial_process_id == $fld->judicial_process_id){
            echo $this->public->get_form_name($fld->form_used); 
            if($fld->form_name != ''){
            ?>
      ...
      $m=1; foreach($latigants as $lt):
            ...
            if($jcf != '0'){
              $j=1; foreach($forms as $fm): 
                if($lt->lit_id == $appuid && $lt->judicial_process_id == $fm->judicial_process_id){ 
      ...
      $m=1; foreach($latigants as $lt):
            ...
            if($jcf != '0'){
              ...
              if($fm->cack_app == '2'){ ?><font color="red">Rejected</font><?php }
