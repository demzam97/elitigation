...
<tr>
   ...
   <td><strong>Registration Date:</strong></td><td><?php echo $reg_date; ?></td>
   </tr>
   <tr>
   <td><strong>Hearing Date:</strong></td><td><?php echo $hearing_date; ?></td>
   <td><strong>Judge:</strong><input type="button" value="Add" class="css_btn_class" onclick="addJudge()" style="width:70px; height:30px; line-height:20px; float:right;"/></td>
   <td><?php foreach($aJudges as $aj){ echo $this->public->get_UserName($aj->judge_id)."<a href='".site_url('registration/deleteCaseJudgeIncase/'.$aj->id)."' onclick='return confirm(\"Are you sure?\")'><img src='".base_url()."/images/cross.png' style='width:10px; float:right; margin-top:5px;'></a><br>"; } ?></td>
   </tr>
</table>
...
<table style="width:100%; float:left;" cellspacing="10px" class="table table-bordered table-striped">
 ...
 <tr> 
 <td colspan="7"><b>Judicial / Case Activity Forms</b></td>
 </tr>
 <tr><th></th>
     <th colspan="3">Plaintiff/Petitioner/Applicant</th>
     <th colspan="3">Respondent/Defendant</th>
     
 </tr>
  <tr>
    <th>Judicial Form Name</th>
    <th>Uploaded Document</th><th>Upload Date</th><th>Acknowledge</th>
    <th>Uploaded Document</th> <th>Upload Date</th><th>Acknowledge</th>
    
  </tr>
  <?php $i=1; foreach($uploads as $up) {?>
   <tr>
    <td><?php echo $this->public->get_form_name($up->form_used);?></td>
    <td><a href="<?php echo site_url('registration/viewUserSubmit/'.$up->form_name.''); ?>" target="_blank"><?php echo $up->form_name; ?></a></td>
    <td><?php echo $up->updated_on; ?></td>
    <td>
    <?php 
    if($up->form_name !='') {
    if($up->cack_app =='0') { ?>
    <form action="<?php echo site_url('registration/ack_court_app_form'); ?>" method="post">
      <input type="hidden" name="id" value="<?php echo $up->id;?>" />
      <input type="hidden" name="caseid" value="<?php echo $case_id; ?>" />
       <select name="appack" id="appack" onchange="submit();">
       ...
       {
        if($up->cack_app == '1') { echo "Accepted"; echo " "; echo"<br>"; echo " "; echo $up->cack_app_date; }
        if($up->cack_app == '2') { echo "Rejected";  echo " "; echo"<br>"; echo " "; echo $up->cack_app_date;}
       } 
    } 
        ?>
    <td><a href="<?php echo site_url('registration/viewUserSubmit/'.$up->form_name_def.''); ?>" target="_blank"><?php echo $up->form_name_def; ?></a></td>
    <td><?php echo $up->updated_on_def; ?></td>
    <td>
    <?php 
    if($up->form_name_def !=''){
    if($up->cack_def =='0') { ?>
    <form action="<?php echo site_url('registration/ack_court_def_form'); ?>" method="post">
      <input type="hidden" name="id" value="<?php echo $up->id;?>" />
      <input type="hidden" name="caseid" value="<?php echo $case_id; ?>" />
       <select name="defack" id="defack" onchange="submit();">
        <option value="">-- Select one --</option>
        <option value="1">Accept</option>
        <option value="2">Reject</option>
        </select></form>
       ...
       {
        if($up->cack_def == '1') { echo "Accepted"; echo " "; echo"<br>"; echo " "; echo $up->cack_def_date; }
        if($up->cack_def == '2') { echo "Rejected";  echo " "; echo"<br>"; echo " "; echo $up->cack_def_date;}
       } 
    }
        ?>
    </td>
  ...
  <?php $i++; } ?>
<tr> 
<td colspan="7"><b>Petition Copy:</b> &nbsp;&nbsp;<a href="<?php echo site_url('registration/viewUserSubmit_p/'.$petitionfilename.''); ?>" target="_blank"><?php echo $petitionfilename; ?></a></td></tr>
</table>

<table style="width:100%; float:left;" cellspacing="10px" class="table table-bordered table-striped">
<tr> <td colspan="7"> <b>Case Files - Submissions / Resubmissions</b></td></tr>
<tr>
  <td width="2%"><strong>Sl.No</strong></td>
   <td width="15%" ><strong>Upload Date</strong></td> 
   <td width="15%" ><strong>Document Name</strong></td>
   <td width="15%"><strong>View</strong></td>
   <td width="15%"><strong>Uploaded by</strong></td>
   <td width="15%"><strong>Acknowledge</strong></td>
   </tr>
   <?php $i=1; foreach($uploads_1 as $u1) { ?>
   <tr>
   <td><?php echo $i; ?></td>
   <td><?php echo $u1->created_on;?></td> 
   <td><?php echo $u1->document_name;?></td>
   <td>
   <a href="<?php echo site_url('registration/viewUserSubmitCasefiles/'.$u1->file_name.''); ?>" target="_blank"><?php echo $u1->file_name; ?></a>
   </td>
   <td>
   <?php echo $name = $this->public->get_UserName($u1->created_by);?>
   </td>
   <td><?php
   if($u1->ack =='0') { ?>
   <form action="<?php echo site_url('registration/ack_submissions'); ?>" method="post">
      <input type="hidden" name="id" value="<?php echo $u1->id;?>" />
      <input type="hidden" name="caseid" value="<?php echo $case_id; ?>" />
       <select name="ack" id="ack" onchange="submit();">
        <option value="">-- Select one --</option>
        <option value="1">Accept</option>
        <option value="2">Reject</option>
        </select></form>
      ...
      {
       if($u1->ack == '1') { echo "Accepted"; echo " "; echo"<br>"; echo " "; echo $u1->ack_date; }
       if($u1->ack == '2') { echo "Rejected";  echo " "; echo"<br>"; echo " "; echo $u1->ack_date;}
      } 
   ?>  
   </td>
</tr>
...
<table style="width:100%; float:left;" cellspacing="10px" class="table table-bordered table-striped">
<tr> <td colspan="7"> <b>Respondent / Defendant / Witness</b></td></tr>
<tr>
  <td width="2%"><strong>Sl.No</strong></td> 
   <td width="15%" ><strong>Name</strong></td>
   <td width="15%"><strong>CID</strong></td>
   <td width="15%"><strong>Contact No.</strong></td>
   <td width="15%"><strong>eMail</strong></td>
   <td width="15%"><strong>Court Fee</strong></td>
   <td width="15%"><strong>Action</strong></td>
   </tr>
   <?php $i=1; foreach($resdef as $rd) { ?>
   <tr>
   <td><?php echo $i; ?></td>
   <td><?php echo $rd->name; ?></td>
   <td><?php echo $rd->cid; ?></td>
   <td><?php echo $rd->mobile; ?></td>
   <td><?php echo $rd->email; ?></td>
   <td><?php 
       if ($resdefpayment == '0') { ?> <span class="label label-warning">Pending</span> <?php } else {?>
       <span class="label label-success">Paid</span> <?php } ?>
	   </td>
   <td><?php if($rd->summon == '0') { ?>
<a href="<?php echo site_url('registration/call_redef/'.$case_id.'/'.$rd->email.'/'.$rd->cid.'/'.$rd->name.'/'.$rd->mobile.'/'.$rd->respondent_defendant.''); ?>" ><i class="fa fa-envelope"></i>Summon</a>
<?php } else { echo "Summoned";}
$ack = $this->public->get_summon_ack($rd->caseid);
foreach($ack as $ack){ if($ack->ack=='1'){ ?> <br><font color= "blue">Ack on <?php echo $ack->ack_date;?> </font><?php }}
?>
</td>
</tr>
   <?php $i++; } ?>
...
<tr> <td colspan="8"><b>Judicial Process</b></td></tr>
        	...
        	<tr>
                ...
                <th>Forms Used</th>
                <th><i class="fa fa-video-camera" aria-hidden="true"></i>&nbsp;Live Meet</th>
         ...
         <?php $i=1; foreach($judicials as $fld): ?>
			 ...
			 else
			 {
				 ...
				 {
				   echo $formname = $this->public->get_form_name($jfm->form_used)."<br>";
				 }
			 }
			 ?>
             ...
             <td>
                 <a href="index.php/registration/livemeet/<?php echo $fld->judicial_process_id; ?>/<?php echo $case_id; ?>/<?php echo "e"; ?>"><i class="fa fa-video-camera" aria-hidden="true"></i>&nbsp;Configure</a><br /><br />
             
             </td>
             <td>
                 <?php if(!empty($j_form)){ ?>
                 <!-- <a href="index.php/registration/send_mail_forms/<?php echo $case_id; ?>/<?php echo $fld->id; ?>"><i class="fa fa-envelope"></i>Form call</a><br /> -->
                 <a href="index.php/registration/edit_caseActivity_elat/<?php echo $fld->id; ?>"><i class="fa fa-pencil"></i> Edit</a>
                 <a href="index.php/registration/delete_judicialProcess_elat/<?php echo $fld->id; ?>" onclick="return confirm('Are you sure to delete?');"><img src="images/cross.png" style="height:12px;"> Delete</a>
                 <?php } ?>
             </td>
...
<div style="float:right;"><a href="index.php/registration/judgement_elat/<?php echo $case_id; ?>" class="btn btn-danger pull-right"> Case Complete</a> </div>
<?php } ?>
<div style="float:left;">
<a href="index.php/registration/insert_case_activity_elat/<?php echo $case_id; ?>" class="btn btn-primary pull-left">Add Judicial Process</a>
</div>
</div>
</div>
