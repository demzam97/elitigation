...
<div class="msg <?php echo $this->session->flashdata('update') ?>"> <span><?php echo $this->session->flashdata('update_msg') ?></span></div>
  <?php endif; ?>
 <b><div class="panel-heading no-collapse">Village</div></b>
<div  class="pull-right"><input  type="text" name="village_search" id="village_search" class="form-control" placeholder="Type Village Name..."/></div><br /><br />
...
<a href="<?php echo site_url('admin/village/add_new');?>" class="btn btn-primary pull-right">Add New</a>

<script type="text/javascript">
 $('document').ready(function(){
	 $('#all').css('display','none');
	 
	 $('#village_search').keyup(function(){
 		//$('.param').css('color','#000');
	 	var val = $(this).val();
		$.ajax({
			type: "POST",
			url : "<?php echo site_url('admin/village/search');?>",
			data: {"value":val},
			dataType : 'html',
